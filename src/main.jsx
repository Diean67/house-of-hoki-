...
const extraCopy = {
  en: {
    language:'Language', region:'Region', regions:{global:'International',de:'Germany',am:'Armenia'}, placeholderWordmark:'Placeholder wordmark',
  ...
  hy: {
    language:'Լեզու', region:'Տարածաշրջան', regions:{global:'Միջազգային',de:'Գերմանիա',am:'Հայաստան'}, placeholderWordmark:'Ժամանակավոր անվանում', trust:['Անգլերեն · Գերմաներեն · Հայերեն','Առցանց և առկա','Գաղտնիությունը՝ առաջնային'], per60:'/ 60 րոպե', free:'Անվճար', pilotActivities:'Փորձնական միջոցառումներ', statuses:['Փորձնական · Հայաստան','Գերմանիա · շուտով','Հայաստան · շուտով'], locationDetails:['Անհատական հանդիպումներ · աշխատանքային օրերին','Ծրագրեր · համայնք · հանդիպումներ','Անգլերեն · Գերմաներեն · Հայերեն'], ctaKicker:'Առաջին քայլը կարող է փոքր լինել',
...
const regionDetector={name:'regionCookie',lookup(){const match=document.cookie.match(/(?:^|; )hoki_region=([^;]*)/);return match?({de:'de',am:'hy',global:'en'}[decodeURIComponent(match[1])]||null):null}}
const detector=new LanguageDetector()
detector.addDetector(regionDetector)
i18n.use(detector).use(initReactI18next).init({resources:Object.fromEntries(Object.entries(copy).map(([key,value])=>[key,{translation:value}])),fallbackLng:'en',supportedLngs:['en','de','hy'],load:'languageOnly',interpolation:{escapeValue:false},detection:{order:['cookie','regionCookie','navigator'],lookupCookie:'hoki_lang',caches:['cookie'],cookieMinutes:525600,cookieOptions:{path:'/',sameSite:'lax'}}})
...
function Header({ lang, setLang, region, setRegion }) {
  ...
  return <header className="header"><Link className="logo-link" to="/"><Logo/></Link><nav aria-label="Main navigation">{nav.map((n,i)=><Link key={n} to={paths[i]}>{n}</Link>)}</nav><div className="header-actions"><label className="sr-only" htmlFor="region">{t('region')}</label><select id="region" value={region} onChange={e=>setRegion(e.target.value)}><option value="global">{t('regions.global')}</option><option value="de">{t('regions.de')}</option><option value="am">{t('regions.am')}</option></select><label className="sr-only" htmlFor="lang">{t('language')}</label><select id="lang" value={lang} onChange={e=>setLang(e.target.value)}><option value="en">EN</option><option value="de">DE</option><option value="hy">HY</option></select><Link className="button small" to="/booking">{t('book')}</Link></div></header>
}
...
function Site() {
  ...
  const readRegion=()=>document.cookie.match(/(?:^|; )hoki_region=([^;]*)/)?.[1]||({de:'de',hy:'am'}[lang]||'global')
  ...
  const setLang=value=>{i18n.changeLanguage(value);saveCookie('hoki_lang',value)}
  const setRegion=value=>{setRegionState(value);saveCookie('hoki_region',value);setLang({de:'de',am:'hy',global:'en'}[value])}
  ...
  return <><ScrollManager/><Header lang={lang} setLang={setLang} region={region} setRegion={setRegion}/><Routes><Route path="/" element={<HomePage lang={lang}/>}/><Route path="/sessions" element={<ListingPage type="sessions"/>}/><Route path="/activities" element={<ListingPage type="activities"/>}/><Route path="/programs" element={<ProgramsPage/>}/>{Object.entries(routePages[lang]||routePages.en).map(([path,page])=><Route key={`${lang}-${path}`} path={path} element={<DetailPage page={page}/>}/>)}<Route path="/booking" element={<BookingPage lang={lang}/>}/><Route path="/orientation-call" element={<OrientationPage/>}/><Route path="/contact" element={<ContactPage/>}/><Route path="/emergency-support" element={<EmergencyPage/>}/><Route path="/about" element={simple('about')}/><Route path="/resources" element={simple('resources',true)}/><Route path="/privacy" element={simple('privacy')}/><Route path="/cookies" element={simple('cookies')}/><Route path="/imprint" element={simple('imprint')}/><Route path="*" element={<Navigate to="/" replace/>}/></Routes><Footer/></>
}

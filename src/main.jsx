...
const extraCopy = {
  en: {
    ...
    cookieBanner:{title:'Your privacy choices',text:'We use essential cookies for the website to work. With your permission, we also remember your language and region on this device. Analytics and marketing cookies are currently disabled.',preferences:'Allow preferences',essential:'Essential only'},
    trust:['English · German · Armenian','Online & in person','Private by design'], per60:'/ 60 min', free:'Free', pilotActivities:'Pilot activities', statuses:['Pilot · Armenia','Germany · coming soon','Armenia · coming soon'],
  ...
  hy: {
    ...
    pages:{about:['Մեր մասին','Խոհուն աճի տուն։','House of Hoki-ն զարգանում է որպես անհատական հանդիպումների, ընդհանուր միջոցառումների և անկախ համայնքային ծրագրերի հարթակ։','Սառայի մասնագիտական վերջնական կոչումը, կենսագրությունն ու որակավորումները կհրապարակվեն հաստատվելուց հետո։'],resources:['Նյութեր','Օգտակար մտքեր՝ խնամքով ներկայացված։','Հոդվածները, հաճախ տրվող հարցերն ու գործնական նյութերը կավելացվեն խմբագրական և մասնագիտական ստուգումից հետո։'],privacy:['Իրավական նախագիծ','Գաղտնիության ծանուցում','Սա կառուցվածքային տեղապահ է։ Նախքան ամրագրման, հարցաշարի, վերլուծության կամ վճարման տվյալներ հավաքելը անհրաժեշտ է GDPR-ով ստուգված ծանուցում։'],cookies:['Իրավական նախագիծ','Cookie կարգավորումներ','Անհրաժեշտ cookie-ները հիշում են լեզուն և տարածաշրջանը։ Վերլուծական և գովազդային cookie-ները անջատված են մինչև համաձայնության համակարգի ներդրումը։'],imprint:['Իրավական նախագիծ','Իրավական տեղեկություններ','Պատասխանատու անձի, հասցեի, կապի և իրավական տվյալները պետք է լրացվեն մինչև հրապարակումը։']}, footerLinks:['Ծառայություններ','Ծրագրեր','Նյութեր','Կապ'],legalLinks:['Գաղտնիություն','Cookie-ներ','Իրավական տվյալներ'],ageNote:'Ընդհանուր ծառայությունները՝ 18+'
  }
}

Object.keys(copy).forEach(language=>Object.assign(copy[language],extraCopy[language]))
const regionDetector={name:'regionCookie',lookup(){const match=document.cookie.match(/(?:^|; )hoki_region=([^;]*)/);return match?({de:'de',am:'hy'}[decodeURIComponent(match[1])]||null):null}}
const detector=new LanguageDetector()
detector.addDetector(regionDetector)
i18n.use(detector).use(initReactI18next).init({resources:Object.fromEntries(Object.entries(copy).map(([key,value])=>[key,{translation:value}])),fallbackLng:'en',supportedLngs:['en','de','hy'],load:'languageOnly',interpolation:{escapeValue:false},detection:{order:['cookie','regionCookie','navigator'],lookupCookie:'hoki_lang',caches:[]}})
...
function Header({ lang, setLang, region, setRegion }) {
  const { t }=useTranslation()
  const paths = ['/sessions','/activities','/programs','/resources','/about']
  const nav=t('nav',{returnObjects:true})
  return <header className="header"><Link className="logo-link" to="/"><Logo/></Link><nav aria-label="Main navigation">{nav.map((n,i)=><Link key={n} to={paths[i]}>{n}</Link>)}</nav><div className="header-actions"><label className="sr-only" htmlFor="region">{t('region')}</label><select id="region" value={region} onChange={e=>setRegion(e.target.value)}><option value="" disabled>{t('chooseRegion')}</option><option value="de">{t('regions.de')}</option><option value="am">{t('regions.am')}</option></select><label className="sr-only" htmlFor="lang">{t('language')}</label><select id="lang" value={lang} onChange={e=>setLang(e.target.value)}><option value="en">EN</option><option value="de">DE</option><option value="hy">HY</option></select><Link className="button small" to="/booking">{t('book')}</Link></div></header>
}
...
function HomePage({ lang }) {
  ...
  const t = copy[lang]
  return <main>
      <section className="hero"><div className="hero-copy"><p className="eyebrow">{t.eyebrow}</p><h1>{t.titleA}<em>{t.titleB}</em></h1><p className="lead">{t.intro}</p><div className="button-row"><Link className="button" to="/booking">{t.book}<span>→</span></Link><Link className="text-link" to="/sessions">{t.explore}<span>→</span></Link></div><div className="trust-row">{t.trust.map(item=><span key={item}>{item}</span>)}</div></div><Art/></section>
...
function Footer() { const {t}=useTranslation(); const footerLinks=t('footerLinks',{returnObjects:true}); const legal=t('legalLinks',{returnObjects:true}); return <footer><div><Logo/><p>{t('footer')}</p></div><div className="footer-links"><Link to="/sessions">{footerLinks[0]}</Link><Link to="/programs">{footerLinks[1]}</Link><Link to="/resources">{footerLinks[2]}</Link><Link to="/contact">{footerLinks[3]}</Link></div><div className="footer-meta"><span>© 2026 House of Hoki</span><span><Link to="/privacy">{legal[0]}</Link> · <Link to="/cookies">{legal[1]}</Link> · <Link to="/imprint">{legal[2]}</Link></span><strong>{t('ageNote')}</strong></div></footer> }

function CookieBanner({onChoose}) { const {t}=useTranslation(); return <aside className="cookie-banner" role="dialog" aria-modal="false" aria-labelledby="cookie-title"><div><strong id="cookie-title">{t('cookieBanner.title')}</strong><p>{t('cookieBanner.text')}</p></div><div className="cookie-actions"><button className="text-button" onClick={()=>onChoose('essential')}>{t('cookieBanner.essential')}</button><button className="button small" onClick={()=>onChoose('preferences')}>{t('cookieBanner.preferences')}</button></div></aside> }

function Site() {
  const {t,i18n}=useTranslation()
  const lang=i18n.resolvedLanguage||'en'
  const readCookie=name=>document.cookie.match(new RegExp(`(?:^|; )${name}=([^;]*)`))?.[1]
  const savedRegion=readCookie('hoki_region')
  const readRegion=()=>['de','am'].includes(savedRegion)?savedRegion:({de:'de',hy:'am'}[lang]||'')
  ...
  const setRegion=value=>{setRegionState(value);if(cookieChoice==='preferences')saveCookie('hoki_region',value);setLang({de:'de',am:'hy'}[value])}
  const chooseCookies=choice=>{setCookieChoice(choice);saveCookie('hoki_cookie_choice',choice);if(choice==='preferences'){saveCookie('hoki_lang',lang);if(region)saveCookie('hoki_region',region)}else{removeCookie('hoki_lang');removeCookie('hoki_region')}}
  ...
  return <><ScrollManager/><Header lang={lang} setLang={setLang} region={region} setRegion={setRegion}/><Routes><Route path="/" element={<HomePage lang={lang}/>}/><Route path="/sessions" element={<ListingPage type="sessions"/>}/><Route path="/activities" element={<ListingPage type="activities"/>}/><Route path="/programs" element={<ProgramsPage/>}/>{Object.entries(routePages[lang]||routePages.en).map(([path,page])=><Route key={`${lang}-${path}`} path={path} element={<DetailPage page={page}/>}/>)}<Route path="/booking" element={<BookingPage lang={lang}/>}/><Route path="/orientation-call" element={<OrientationPage/>}/><Route path="/contact" element={<ContactPage/>}/><Route path="/emergency-support" element={<EmergencyPage/>}/><Route path="/about" element={simple('about')}/><Route path="/resources" element={simple('resources',true)}/><Route path="/privacy" element={simple('privacy')}/><Route path="/cookies" element={simple('cookies')}/><Route path="/imprint" element={simple('imprint')}/><Route path="*" element={<Navigate to="/" replace/>}/></Routes><Footer/>{!cookieChoice&&<CookieBanner onChoose={chooseCookies}/>}</>
}
